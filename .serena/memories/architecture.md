...
2. **3x-ui Client** (`internal/xui/client.go`)
   ...
   - Retry with jitter
   - Множественные источники: `map[uint]interfaces.XUIClient`

3. **Service Layer** (`internal/service/`)
   - `SubscriptionService` — оркестрация создания trial/подписок
   - `CreateTrial` — итерация по всем trial-источникам (best-effort)
   - `BindTrial` — обновление клиента на первом успешном источнике
   - `ReconcileOrphanedClients` — проверка всех источников (удаление только если клиент не найден ВЕЗДЕ)
...
### 🟡 Ограничение #1: 3x-ui = несколько серверов (частично решено)

**v2.4.0 — Multi-source migration**
- Добавлена поддержка множества 3x-ui панелей (источников)
- Каждый источник: свой 3x-ui URL, API-токен, inbound ID
- Trial-подписки создаются на всех trial-источниках
- BindTrial обновляет клиента на первом успешном источнике
- ReconcileOrphanedClients проверяет все источники

**Ограничения:**
- Пока нет multi-server конфига (один сервер на подписку)
- Клиент создаётся на inbound одного источника
- Нет failover между источниками
- Нет балансировки нагрузки

**Требование бизнеса (на будущее):**
```
...
### Решение #1: Multi-source 3x-ui (текущее)

**Статус:** ✅ РЕАЛИЗОВАНО  
**Дата:** 2026-05-26

**Multi-source архитектура:**
- `config.Source` — структура с URL, API-токеном, inbound ID, флагами Trial/Active
- Транспорт: `xui.NewHTTPClient` для каждого источника
- Сервис: `map[uint]interfaces.XUIClient` + `[]database.Source`
- Trial: создаётся на всех источниках с `Trial:true`
- BindTrial: первый успешный источник (пока один)
- Orphaned cleanup: проверка всех источников

**Config:**
- `DEFAULT_SOURCE_SUB_URL` — base URL для подписок источника
- `GLOBAL_SUB_URL` — глобальный URL (subproxy)
- Множественные секции `PANEL_1_*`, `PANEL_2_*` в `.env`
- Fallback на старые переменные `XUI_HOST`/`XUI_API_TOKEN`/`XUI_INBOUND_ID`

### Решение #2: Кастомный генератор подписок (отложено)

**Статус:** ⏸ ОТЛОЖЕНО  
**Дата:** 2026-04-02 (отложено 2026-05-26 при multi-source миграции)
