...
func (s *Service) UpdateProductGuarded(ctx context.Context, product *Product) error {
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		err := tx.First(&current, product.ID).Error
		if err != nil {
		...
		err = tx.Model(&Product{}).Where("id = ?", product.ID).Update("is_active", current.IsActive).Error
		if err != nil {
		...
		err = tx.Model(&Order{}).Where("product_id = ?", product.ID).Count(&orderCount).Error
		if err != nil {
		...
		result := tx.Model(&Product{}).Where("id = ?", product.ID).Updates(map[string]any{
