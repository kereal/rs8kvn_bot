...
// UpdateRemindersSent atomically sets reminder bits for a subscription.
func (s *Service) UpdateRemindersSent(ctx context.Context, id uint, bit int) error {
	result := s.db.WithContext(ctx).Model(&Subscription{}).
		Where("id = ?", id).
		Update("reminders_sent", gorm.Expr("reminders_sent | ?", bit))
	if result.Error != nil {
		return fmt.Errorf("failed to update reminders_sent: %w", result.Error)
	}
	if result.RowsAffected == 0 {
		return ErrSubscriptionNotFound
	}
