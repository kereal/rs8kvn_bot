...
-- Description: Убирает статус pending_update из CHECK constraint таблицы subscription_nodes.
...
SELECT
    subscription_id,
    node_id,
    CASE WHEN status = 'pending_update' THEN 'pending_add' ELSE status END,
    retry_count,
    retry_at,
    last_error,
    updated_at
FROM subscription_nodes_old;
