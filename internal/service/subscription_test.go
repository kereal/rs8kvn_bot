...
func TestSubscriptionService_SendExpiryReminder_SendsMessageAndMarksBit(t *testing.T) {
	...
	err = svc.SendExpiryReminder(ctx, sub, ReminderBit3Days, 3, 10)
	require.NoError(t, err)
...
func TestSubscriptionService_SendExpiryReminder_NilBotNoop(t *testing.T) {
	...
	err = svc.SendExpiryReminder(ctx, sub, ReminderBit1Day, 1, 0)
	require.NoError(t, err)
...
// TestSubscriptionService_SendExpiryReminder_DBUpdateErrorPropagates verifies that
// a failure to mark the reminder bit is returned to the caller even after the
// Telegram message was successfully sent.
func TestSubscriptionService_SendExpiryReminder_DBUpdateErrorPropagates(t *testing.T) {
	t.Parallel()
	...
	db.UpdateRemindersSentFunc = func(ctx context.Context, id uint, bit int) error {
		return updateErr
	}

	cfg := &config.Config{GlobalSubURL: "https://vpn.example.com/sub/"}
	svc := NewSubscriptionService(db, nil, nil, nil, cfg)
	svc.SetBot(testutil.NewBotAPI())

	err := svc.SendExpiryReminder(ctx, sub, ReminderBit3Hours, 0, 3)
	require.Error(t, err)
	assert.ErrorIs(t, err, updateErr)
}

// TestSubscriptionService_RenewSubscription_ResetsRemindersSent verifies the
// reminder lifecycle invariant: after a successful renewal the subscription
// re-enters the reminder cycle with RemindersSent = 0 (new window, new touches).
...
func TestSubscriptionService_SendExpiryReminder_HoursOnlyText(t *testing.T) {
	...
	sub := &database.Subscription{
		TelegramID:     888,
		Username:       "hour_user",
		ClientID:       "client-hour",
		SubscriptionID: "sub-hour",
		Status:         "active",
		PlanID:         1,
		ExpiresAt:      &expiry,
	}
	...
	sent := bot.GetAllSentMessages()
	require.Len(t, sent, 1)
	assert.Contains(t, sent[0].Text, "3 ч")
}
