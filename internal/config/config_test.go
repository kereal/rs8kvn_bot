...
func TestConfig_String(t *testing.T) {
	...
	assert.NotContains(t, str, "some-token", "String() should mask API token")
}
...
func TestConfig_Validate_EmptyXUIAPIToken(t *testing.T) {
	t.Parallel()
	...
	err := cfg.validate()
	assert.Error(t, err, "validate() should error on empty XUIAPIToken")
}
...
func FuzzLoad_InvalidEnvValues(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, invalidVal string) {
		...
		defer func() {
			for _, key := range []string{"TELEGRAM_ADMIN_ID", "XUI_INBOUND_ID", "TRAFFIC_LIMIT_GB", "HEARTBEAT_INTERVAL", "TRIAL_DURATION_HOURS", "TRIAL_RATE_LIMIT", "HEALTH_CHECK_PORT", "TELEGRAM_BOT_TOKEN", "TELEGRAM_ADMIN_ID", "XUI_HOST", "XUI_INBOUND_ID", "LOG_LEVEL"} {
				os.Unsetenv(key)
			}
		}()
