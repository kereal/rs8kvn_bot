...
func TestHandleStart_WithTrialCode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := newTestHandlerWithSubService(t, tt.cfg, mockDB, mockXUI, mockBot)
			tt.setupMock(mockDB)
...
func TestHandleBindTrial_WithReferrerNotification(t *testing.T) {
	...
	mockXUI := testutil.NewMockXUIClient()
	mockBot := testutil.NewMockBotAPI()
	handler := newTestHandlerWithSubService(t, cfg, mockDB, mockXUI, mockBot)
...
func TestHandleBindTrial_Success(t *testing.T) {
	...
	dbSources := []database.Source{{
		...
		XUIAPIToken: "token", XUIInboundID: 1, Active: true, Trial: true,
	}}
	mockXUI := testutil.NewMockXUIClient()
...
func TestHandleBindTrial_UpdateClientError(t *testing.T) {
	...
	mockXUI.UpdateClientFunc = func(ctx context.Context, inboundID int, currentEmail, clientID, email, subID string, trafficBytes int64, expiryTime time.Time, telegramID int64, comment string) error {
...
func newTestHandlerWithSubService(t *testing.T, cfg *config.Config, mockDB *testutil.MockDatabaseService, mockXUI *testutil.MockXUIClient, mockBot *testutil.MockBotAPI) *Handler {
	t.Helper()
	if cfg == nil {
		cfg = &config.Config{TrafficLimitGB: 50}
	}
	if mockDB == nil {
		mockDB = testutil.NewMockDatabaseService()
	}
