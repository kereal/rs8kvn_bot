...
|---|---|---|
| `xui_requests_total{operation,result}` | counter | Объявлена для подсчёта запросов к 3x-ui панели, но ни `xui/`, ни `subserver/fetch.go` её не инкрементируют. `FetchFromSource` делает прямые `http.Client.Do` без метрик. |
...
fetch.go (FetchFromSource) ─ xui_requests_total{operation,result}  мёртвая
