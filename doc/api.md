...
`CONFIRMED` atomically changes `pending → paid`, updates the subscription and creates DB sync prerequisites in one transaction. Post-commit VPN sync and Telegram delivery are best-effort and do not roll back the payment. `CANCELED` cancels only pending orders. `CHARGEBACKED` cancels pending/paid orders; a chargeback on a previously-paid order automatically downgrades the subscription to the free plan (unless another paid order for the same subscription exists — access is then preserved for manual review) and sends an administrator alert with order, user, amount, currency, and provider transaction data.
