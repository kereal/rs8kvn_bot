...
| `SUBSERVER_ACCESS_LOG` | `/sub/{id}` access log file path | `./data/subserver.log` | ❌ | Set empty to disable; uses the same structured zap-console format as the main log; startup continues if the file cannot be opened |
| **Trial & Referral** |
