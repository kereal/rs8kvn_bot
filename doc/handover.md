...
- **Admin alerts (best-effort Markdown, `TelegramAdminID`):** on activation (`Activated=true`, only the pending→paid caller) `notifyAdminPaid` — tariff, formatted amount, clickable buyer link (`utils.FormatUserLink`), `🆕 Покупка` vs `🔄 Продление` (from `PricePaidCents`/`ProductID` captured before CAS); on paid-order chargeback (`WasPaid=true`) `notifyAdminChargeback` — access status (downgraded to free / preserved). Every integration problem (mismatch, late/unknown callback, provider error, uncertain outcome) → `NotifyPaymentIssue` → `notifyAdmin` + `payment_issues_total{event}`. Duplicate callbacks are silent.
